...
interface Env {
  ...
  lala_voice_agent_db: D1Database;
}
...
async function lookupOrder(orderId: string, db?: D1Database) {
  ...
  try {
    ...
    if (db) {
      const result = (await db
        .prepare(
          `SELECT o.id, o.order_number, o.status, o.total, o.shipping_address, o.created_at,
                  GROUP_CONCAT(oi.quantity || 'x ' || p.name || ' ($' || oi.price || ')', ', ') as items
           FROM orders o
      ...
      if (result) {
        return {
          found: true,
          id: result.order_number as string,
          status: result.status as string,
          items: result.items ? (result.items as string).split(", ") : [],
          total: Number(result.total).toFixed(2),
          shipping_address: result.shipping_address as string,
...
async function checkProduct(productName: string, db?: D1Database) {
  ...
  try {
    ...
    if (db) {
      ...
      const result = (await db
        .prepare(
          `SELECT id, name, brand, category, price, stock FROM products
      ...
      if (result) {
        return {
          ...
          stock_count: result.stock as number,
        };
      }
    }
...
function buildTools(db?: D1Database) {
  return {
    lookup_order: tool({
      description:
      ...
      execute: async ({ order_id }) => lookupOrder(order_id, db),
    }),
    request_return: tool({
      description:
    ...
    check_product_availability: tool({
      ...
      execute: async ({ product_name }) => checkProduct(product_name, db),
    }),
  };
}
...
const SYSTEM_PROMPT = `You are Lala, a friendly and professional customer support voice assistant.
...
export class VoiceAgent extends BaseVoiceAgent<Env> {
  ...
  async onCallStart() {
    for (const connection of this.getConnections()) {
      await this.speak(
        connection,
        "Hi! I'm Lala, your personal voice assistant. I can help you track your orders, start a return, or check product availability. What can I help you with today?",
      );
    }
  }
  ...
  async onTurn(transcript: string, context: VoiceTurnContext) {
    ...
    const tools = buildTools(this.env.lala_voice_agent_db);
